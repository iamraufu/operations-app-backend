...
const poReleased = async (req, res) => {
      try {
            ...
            const response = await fetch(`${process.env.SAP_PROD}get_po_released.php`, requestOptions)
            const data = await response.json()
...
const poDisplay = async (req, res) => {
      try {
            ...
            const response = await fetch(`${process.env.SAP_PROD}po_display.php`, requestOptions)
            const data = await response.json()
            ...
            if (data === 'Could not open connection') {
                  ...
                  res.status(200).json({
                        ...
                        //       supplyingPlant: data.PO_HEADER.VENDOR.trim(),
                        ...
                        // Current Details from PO Display BAPI
                        data: {
                              po: data.POHEADER.PO_NUMBER.trim(),
